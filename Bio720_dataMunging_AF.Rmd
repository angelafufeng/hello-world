...
```{r}
missing_data=sum(is.na(dll_data))
missing_data
```
...
### using the index
```{r}
dll_data_wt <- dll_data[dll_data$genotype == "wt",] #comma tells it that it's by row
dll_data_wt
```
...
# R will keep the information that these observations could occur
```
...
# a factor is stored as an integer typeof(), but we want to turn it into a character
...
```{r}
temp_as_factor2 <- with(dll_data, ifelse(temp == 25, "LowTemp", ifelse(temp==30, "HighTemp", NA))
